...
class ValidationResult:
    ...
    def __repr__(self):
        ...
        return f"{status} [{self.level.value}] {self.message}"
...
class OrderValidator:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.max_position_size = self.config.get('max_position_size', 100000)
        self.max_daily_orders = self.config.get('max_daily_orders', 50)
        self.max_price_deviation = self.config.get('max_price_deviation', 0.05)  # 5%
        self.max_exposure_per_asset = self.config.get('max_exposure_per_asset', 0.3)  # 30%
    ...
    def _validate_exposure(
        ...
        current_exposure = positions.get(symbol, {}).get('value', 0) if isinstance(positions.get(symbol), dict) else 0
    ...
    def _log_validation_results(
        ...
        logger.info(f"Símbolo: {order.get('symbol')} | Lado: {order.get('side')} | Cantidad: {order.get('quantity')}")
        logger.info(f"{'='*60}")
    ...
    def get_validation_summary(self) -> Dict:
        ...
        errors = sum(1 for r in self.validation_history if not r.passed and r.level == ValidationLevel.ERROR)
        warnings = sum(1 for r in self.validation_history if not r.passed and r.level == ValidationLevel.WARNING)
